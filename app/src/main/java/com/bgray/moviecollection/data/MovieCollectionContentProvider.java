...
import com.bgray.moviecollection.R;
...
public class MovieCollectionContentProvider extends ContentProvider {
   ...
   static {
      ...
      uriMatcher.addURI(DatabaseDescription.AUTHORITY,
         Movie.TABLE_NAME + "/#", ONE_CONTACT);

      // Uri for Contacts table
      uriMatcher.addURI(DatabaseDescription.AUTHORITY,
         Movie.TABLE_NAME, CONTACTS);
   }

   // called when the MovieCollectionContentProvider is created
   @Override
      ...
      switch (uriMatcher.match(uri)) {
         case CONTACTS:
            ...
            if (rowId > 0) { // SQLite row IDs start at 1
               newContactUri = Movie.buildMovieUri(rowId);
